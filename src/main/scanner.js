...
const moduleDetector = require('./modules/core/moduleDetector');
...
/**
 * Oyun klasöründe upscaler yeteneklerini ve kurulu modları tespit eder.
 *
 * Dönüş: { dlss, xess, fsr, detections: { <modulId>: { dir, injection, version, depth } } }
 * Mod tespiti tamamen manifest'lerin `detect` bloğundan sürülür (moduleDetector);
 * burada mod adı geçen sabit bir liste YOK.
 */
async function detectUpscalers(gamePath) {
    const result = { dlss: false, xess: false, fsr: false, detections: {} };
    if (!fs.existsSync(gamePath)) return result;

    moduleManager.ensureInit();
    const detectors = moduleDetector.getDetectors();
    const matchPlan = moduleDetector.buildMatchPlan(detectors);
    ...
    while (queue.length > 0) {
        ...
        try {
            ...
            // requireSiblingFile kontrolü için klasör içeriği (dizin başına bir kez)
            let dirFileSet = null;
            const hasSibling = (name) => {
                if (!dirFileSet) {
                    dirFileSet = new Set(files.filter(f => !f.isDirectory()).map(f => f.name.toLowerCase()));
                }
            ...
            for (const file of files) {
                ...
                if (file.isFile()) {
                    ...
                    // ── Manifest tabanlı mod tespiti ────────────────────────────
                    // Pahalı okumalar (FileDescription / FileVersion) yalnızca
                    // manifest'lerin ilgilendiği dosya adlarında yapılır.
                    if (matchPlan.isCandidate(nameLow)) {
                        ...
                        if (hit) {
                            const previous = result.detections[hit.moduleId];
                            const deepest = hit.detector.detect.depthStrategy === 'deepest';
                            // Varsayılan: BFS'te ilk (en sığ) bulunan kazanır.
                            // 'deepest' → daha derin bir klasörde bulunursa üzerine yazılır (Streamline).
                            if (!previous || (deepest && current.depth > previous.depth)) {
                                result.detections[hit.moduleId] = {
                                    dir: current.path,
                                    injection: hit.viaProxy ? file.name : (previous ? previous.injection : null),
                                    version: hit.version || (previous ? previous.version : null),
                                    depth: current.depth
                                };
                            } else if (previous && !previous.version && hit.version) {
            ...
            // Optimization: tüm sinyaller ve tüm modüller bulunduysa taramayı bitir.
            // 'deepest' stratejili modüller (Streamline) daha derinde güncellenebileceği
            // için erken çıkışa dahil edilmez.
            if (result.dlss && result.xess && result.fsr &&
                detectors.every(d => d.detect.depthStrategy === 'deepest' || (result.detections[d.moduleId] && result.detections[d.moduleId].version))) {
                moduleDetector.resolveExclusives(result.detections, detectors);
                return result;
            }
        } catch (e) { }
    ...
    // Aynı exclusiveGroup'tan yalnızca en yüksek öncelikli tespit kalsın
    // (ör. optiscaler / optibuilder / optiscaler-dlssnr aynı DLL'i paylaşır)
    moduleDetector.resolveExclusives(result.detections, detectors);
    return result;
}
...
async function processAndStreamGame(game, event, scanSettings) {
    ...
    if (existingGame) {
        ...
        // 3. Mod Durumlarını Aktar — tamamen manifest'lerin state alanlarından sürülür.
        // Hangi alanın yazılacağını moduleDetector, manifest.state üzerinden belirler.
        ...
        moduleDetector.applyDetections(existingGame, detectedUpscalers.detections);

        // Manifest kapsamı dışındaki ek Streamline alanları
        ...
        const finalGame = {
            ...
            // Mod bayrakları/sürümleri manifest'lerin state alanlarından doldurulur
            upscalers: {
                dlss: detectedUpscalers.dlss || false,
                xess: detectedUpscalers.xess || false,
                fsr: detectedUpscalers.fsr || false
            },
            isFavorite: isFavorite
        };
        moduleDetector.applyDetections(finalGame, detectedUpscalers.detections);
