...
async function installAddons(parentManifest, options, destDir, logger) {
    ...
    if (!Array.isArray(parentManifest.addons) || !requested) {
        return { installed, warnings, configChanges };
    }
    ...
    for (const addon of parentManifest.addons) {
        ...
        try {
            ...
            logger.step(`Eklenti kuruldu: ${addonLabel} (${tag}) — ${copied} dosya → ${path.basename(addonDest)}`);
...
async function install(manifest, gameName, exePath, tag, options, onProgress = () => {}) {
    ...
    try {
        ...
        if (Array.isArray(manifest.requires) && manifest.requires.length > 0) {
            ...
            logger.step('Ön koşullar sağlandı');
        }
        ...
        if (manifest.install?.proxyDetection) {
            ...
            // Öncelik: kullanıcının modaldaki seçimi > diskte tespit edilen mevcut proxy > manifest varsayılanı
            ...
            if (isRequestedValid) {
                effectiveProxyTarget = requestedTarget;
                logger.step(`Proxy DLL hedeflendi: ${effectiveProxyTarget} (kullanıcı seçimi)`);
            } else {
        ...
        if (manifest.install?.apiTargeting) {
            ...
            logger.step(`API hedefleme: ${arch} / ${chosenApi} -> ${sourceFile} kopyalanacak ad: ${targetName}`);
            if (detection.notes && detection.notes.length > 0) {
                logger.step(`Tespit ipuclari: ${detection.notes.join(', ')}`);
            }
        }
        ...
        // sessizce bozulur ve kaldırılınca geri gelmez. Kurulum burada durur.
        {
            ...
            if (owner) {
                ...
                logger.warn(`Çakışma: ${owner.fileName} dosyası ${owner.moduleName} moduna ait.`);
                throw err;
            }
        }
            ...
            if (apiTarget) {
                ...
                if (fs.existsSync(destFile)) {
                    ...
                    try {
                        ...
                        logger.warn(`Mevcut ${apiTarget.targetName} yedeklendi: ${path.basename(backupPath)}`);
                    } catch (backupErr) {
                        logger.warn(`Mevcut dosya yedeklenemedi (${backupErr.message}), uzerine yaziliyor.`);
                    }
                }
                ...
                logger.step(`${apiTarget.sourceFile} -> ${apiTarget.targetName} kopyalandi`);
            } else {
            ...
            for (const extra of manifest.install.extraSources) {
                ...
                try {
                    ...
                    if (extra.skipIfExists && fs.existsSync(path.join(destDir, extra.skipIfExists))) {
                        logger.step(`Ek kaynak zaten mevcut, atlandı: ${extraLabel}`);
                        continue;
                    }
                    ...
                    try {
                        ...
                        for (const item of wanted) {
                            ...
                            if (!fs.existsSync(src)) {
                                logger.warn(`Ek kaynakta bulunamadı, atlandı: ${item}`);
                                continue;
                            }
                        ...
                        logger.step(`Ek kaynak kuruldu: ${extraLabel} → ${extra.extractTo || '.'} (${extraCopied} öğe)`);
                    } finally {
                        try { fs.rmSync(extraTmp, { recursive: true, force: true }); } catch (e) { /* yoksay */ }
                    }
                } catch (extraErr) {
                    // Ek kaynak hatası ana kurulumu bozmaz
                    logger.warn(`Ek kaynak kurulamadı (${extraLabel}): ${extraErr.message}`);
                }
            }
        }
        ...
        let addonResult = { installed: [], warnings: [], configChanges: {} };
