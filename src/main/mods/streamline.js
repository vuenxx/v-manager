...
async function restoreStreamline(gameName) {
    ...
    if (!targetDir || !fs.existsSync(targetDir)) {
        ...
        // Mod bayrakları manifest'in state alanlarından yazılır (moduleDetector)
        moduleDetector.applyDetections(game, detection.detections);
    ...
    try {
        ...
        // Update game state with detection results (manifest state alanları üzerinden)
        moduleDetector.applyDetections(game, detection.detections);
        delete game.streamlineHashes;
        delete game.streamlineModVersion;
...
async function installStreamline(game, version, targetDir) {
    ...
    try {
        ...
        if (dbGame) {
            moduleDetector.applyDetections(dbGame, detection.detections);
